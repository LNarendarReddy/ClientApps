    ...
    public class VoucherRepository : RepositoryBase<Voucher>
    {
        public override Voucher Load(DataRow dtVoucherRow)
        {
            return new Voucher()
            {
                ...
                ChequeNumber = dtVoucherRow["CHEQUENUMBER"],
                ModeOfTransfer = dtVoucherRow["MODEOFTRANSFER"],
                ChequeRegisterID = dtVoucherRow["CHEQUEREGISTERID"]
            };
        }
        ...
        public Voucher GetVoucherDetails(object VoucherID)
        {
            ...
            try
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.Connection = SQLCon.Sqlconn();
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandText = "[USP_R_VOUCHERDETAILS]";
                    cmd.Parameters.AddWithValue("@VoucherID", VoucherID);
                    using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                    {
                        da.Fill(dtVoucherDetails);
                    }
                }
            }
            catch (Exception ex)
            {
                throw new Exception("Error While Retrieving Voucher Details", ex);
            }
            finally
            {
                SQLCon.Sqlconn().Close();
            }
            return Load(dtVoucherDetails.Rows[0]);
        }
    }
}
