    ...
    public partial class frmVoucher : NavigationBase
    {
        ...
        private void frmVoucherNew_Load(object sender, EventArgs e)
        {
            ...
            if (voucherObj.ID != null)
            {
                ...
                txtChequeNumber.EditValue = voucherObj.ChequeNumber;
                rgModeOfTransfer.SelectedIndex = Convert.ToInt16(voucherObj.ModeOfTransfer);
        ...
        private void Savevoucher(bool IsPrint = false)
        {
            ...
            voucherObj.ModeOfTransfer = rgModeOfTransfer.SelectedIndex;
            ...
            try
            {
                ...
                if (IsPrint)
                {
                    ...
                    rpt.Parameters["PaymentMadeFromCaption"].Value = "Credit to : ";
                    rpt.Parameters["PaymentMadeToCaption"].Value = "Debit to : ";
        ...
        private void btnMultipleVouchers_Click(object sender, EventArgs e)
        {
            ...
            MultiVoucherSettings multiVoucherSettings = new MultiVoucherSettings()
            {
                ...
                IsBankPaymentVoucher = voucherObj.VoucherTypeID.Equals(LookUpIDMap.VoucherType_BankPayment)
            };
            Utility.ShowDialog(new ucMultiVoucher(multiVoucherSettings));
        }

        private void cmbPaymentMadeto_EditValueChanged(object sender, EventArgs e)
        {
            if (isLoading) return;
            if (cmbPaymentMadeto.EditValue == null ||
                cmbPaymentMadeto.EditValue.Equals(Utility.CurrentEntity.CASHINHANDID) ||
                (cmbPaymentMadefrom.EditValue != null 
                    && cmbPaymentMadefrom.EditValue.Equals(Utility.CurrentEntity.CASHINHANDID)))
                return;
            cmbPaymentMadefrom.EditValue = null;
            BindLookups();
            DataTable table = cmbPaymentMadefrom.Properties.DataSource as DataTable;
            DataView clone = new DataView(table);
            clone.RowFilter = string.Format($"[LEDGERID] <> {cmbPaymentMadeto.EditValue}");
            cmbPaymentMadefrom.Properties.DataSource = clone;
        }
    }
}