...
namespace IIT
{
    public partial class ucLedgerReport : NavigationBase
    {
        ...
        private void FetchAndBindDataSource()
        {
            ...
            colPurpose.Visible = CurrentSettings.ShowPurpose == "Yes";
            ColBalance.Visible = CurrentSettings.ShowClosingBalacne == "Yes";
        }
        ...
        private void gvVouchers_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar != (char)Keys.Enter)
                return;
            if (gvVouchers.FocusedRowHandle < 0) return;
            Voucher voucherObj = voucherRepository.GetVoucherDetails(gvVouchers.GetFocusedRowCellValue("VOUCHERID"));
            Utility.ShowDialog(new frmVoucher(voucherObj));
            if (voucherObj.IsSave)
            {
                FetchAndBindDataSource();
                gvVouchers.FocusedRowHandle = gvVouchers.LocateByValue("VOUCHERID", voucherObj.ID);
            }
        }
    }
}
