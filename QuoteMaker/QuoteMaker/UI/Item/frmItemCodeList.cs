...
namespace QuoteMaker.UI.Items
{
    public partial class frmItemCodeList : DevExpress.XtraEditors.XtraForm
    {
        ...
        {
            Item item = new Item();
            new frmItemCode(item) { Owner = this }.ShowDialog();

            Utility.Setfocus(gvItemList, "ITEMCODEID", item.ItemCodeID);

            if(item.IsSave && Convert.ToBoolean(item.IsNewToggleSwitched))
            {
        ...
        private void btnMRPList_Click(object sender, EventArgs e)
        {
            try
            {
                if (gvItemList.FocusedRowHandle >= 0)
                {
                    ...
                    {
                        {"ITEMCODEID",  gvItemList.GetFocusedRowCellValue("ITEMCODEID")}
                    };
                    DataTable dtMRPList = 
        ...
        {
            DataTable dt = new RepositoryBase().GetDataTable("USP_R_EXPORTITEMS", 
